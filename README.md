...
├── scripts/install_xray_panel.sh  # Instala Xray + painel de gerenciamento em Ubuntu
├── scripts/painel                 # Comando `painel XRAY`
├── scripts/xray_panel.py          # Painel shell em Python para UUIDs e links VLESS
├── vercel.json      # Routes all paths → /api/index
└── README.md
```

## Backend Xray Manager for Ubuntu VPS

This repository now also includes a minimal backend manager for a VPS running
Xray with `xhttp` inbound, intended to be used behind this Vercel relay.

### What it installs

- `Xray-core` with support for `x86_64`, `arm64`, and `armv7`
- a root-only shell panel written in Python
- a `systemd` service for Xray
- config/state files under `/etc/xray`

### Features

- create UUIDs
- change UUID validity and automatically drop expired users from active config
- remove UUIDs
- edit backend port and XHTTP path
- edit relay `address`, `sni`, and `host`
- generate VLESS links in the same format as:

```text
vless://26ff30da-8fe1-4d13-9dc8-f08e6c980ffc@prezaofree.monaco.gg:443?path=%2Fapp2%2F&security=tls&encryption=none&insecure=1&host=vercel1-one-mu.vercel.app&fp=chrome&type=xhttp&allowInsecure=1&sni=prezaofree.monaco.gg#xray-vercel-cliente
```
...
```bash
chmod +x scripts/install_xray_panel.sh
sudo ./scripts/install_xray_panel.sh
```
...
The backend Xray created by the panel listens with `xhttp` on your VPS. The
Vercel project in this repository must point to that backend using
`TARGET_DOMAIN`, for example:

```text
TARGET_DOMAIN=http://your-vps-domain-or-ip:2096
```

The relay-side `path` in the VLESS link must match the backend `path` used by
the panel.
